...
// Rotation stuff
float ROT_ANGLE = glm::radians(1.0f);

// Translation stuff
const float TRAN_VALUE = 0.025f;
float STEVE_VALUE = 0.025f;
float CREEPER_VALUE = 0.025f;
...
ShaderProgram g_steve_program;
ShaderProgram g_creeper_program;
glm::mat4 view_matrix, m_model_matrix, m_projection_matrix, m_trans_matrix;
float m_previous_ticks = 0.0f;

GLuint g_steve_texture_id;
GLuint g_creeper_texture_id;
...
glm::mat4 g_view_matrix,        // Defines the position (location and orientation) of the camera
          g_steve_matrix, g_creeper_matrix,      // Defines every translation, rotation, and/or scaling applied to an object; we'll look at these next week
...
void initialise()
{
    ...
    // Load up our shaders
    g_steve_program.Load(V_SHADER_PATH, F_SHADER_PATH);
    g_creeper_program.Load(V_SHADER_PATH, F_SHADER_PATH);
    
    g_steve_matrix      = glm::mat4(1.0f);  // Defines every translation, rotations, or scaling applied to an object
...
//    g_model_matrix = glm::translate(g_model_matrix, glm::vec3(5.0f, 0.0f, 0.0f));
    ...
    g_steve_program.SetProjectionMatrix(g_projection_matrix);
    g_creeper_program.SetProjectionMatrix(g_projection_matrix);
    
    g_steve_program.SetViewMatrix(g_view_matrix);
    g_creeper_program.SetViewMatrix(g_view_matrix);
    ...
    glUseProgram(g_steve_program.programID);
    glUseProgram(g_creeper_program.programID);
    
    // Binds texture id
    g_steve_texture_id = load_texture(PLAYER_SPRITE_FILEPATH);
    g_creeper_texture_id = load_texture(PLAYER_SPRITE_FILEPATH2);
    
    g_creeper_matrix = glm::translate(g_creeper_matrix, glm::vec3(1.0f, 2.0f, 0.0f));
...
void update()
{
    ...
    g_steve_matrix = glm::rotate(g_steve_matrix, ROT_ANGLE, glm::vec3(0.0f, 0.0f, 1.0f));
    g_creeper_matrix = glm::rotate(g_creeper_matrix, ROT_ANGLE, glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    g_steve_matrix = glm::translate(g_steve_matrix, glm::vec3(STEVE_VALUE, 0.0f, 0.0f));

    g_creeper_matrix = glm::translate(g_creeper_matrix, glm::vec3(STEVE_VALUE+2.0f, 0.0f, 0.0f));

    // rotates the shape
    g_steve_matrix = glm::rotate(g_steve_matrix, ROT_ANGLE, glm::vec3(0.0f, 1.0f, 0.0f));

    g_creeper_matrix = glm::rotate(g_creeper_matrix, ROT_ANGLE, glm::vec3(0.0f, 1.0f, 0.0f));

    // scales the shape. EXTRA CREDIT
    g_steve_matrix = glm::scale(g_steve_matrix, scale_vector);
    g_creeper_matrix = glm::scale(g_creeper_matrix, scale_vector);
}
...
void render() {
    ...
    //g_steve_program.SetModelMatrix(g_steve_matrix);
    //g_creeper_program.SetModelMatrix(g_creeper_matrix);
    ...
    // steve object
    glVertexAttribPointer(g_steve_program.positionAttribute, 2, GL_FLOAT, false, 0, vertices);
    glEnableVertexAttribArray(g_steve_program.positionAttribute);
    
    glVertexAttribPointer(g_steve_program.texCoordAttribute, 2, GL_FLOAT, false, 0, texture_coordinates);
    glEnableVertexAttribArray(g_steve_program.texCoordAttribute);
    
    g_steve_program.SetModelMatrix(g_steve_matrix);
    
    glBindTexture(GL_TEXTURE_2D, g_steve_texture_id);
    glDrawArrays(GL_TRIANGLES, 0, 6);
    
    glDisableVertexAttribArray(g_steve_program.positionAttribute);
    glDisableVertexAttribArray(g_steve_program.texCoordAttribute);
    
    // creeper object
    glVertexAttribPointer(g_creeper_program.positionAttribute, 2, GL_FLOAT, false, 0, vertices);
    glEnableVertexAttribArray(g_creeper_program.positionAttribute);
    
    glVertexAttribPointer(g_creeper_program.texCoordAttribute, 2, GL_FLOAT, false, 0, texture_coordinates);
    glEnableVertexAttribArray(g_creeper_program.texCoordAttribute);
    
    g_creeper_program.SetModelMatrix(g_creeper_matrix);
    
    glBindTexture(GL_TEXTURE_2D, g_creeper_texture_id);
    glDrawArrays(GL_TRIANGLES, 0, 6);
